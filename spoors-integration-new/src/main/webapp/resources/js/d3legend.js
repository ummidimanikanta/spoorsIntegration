...
function d3Legend() {
  var margin = {top: 2, right: 0, bottom: 0, left: 10},
      width = 400,
      height = 20,
  ...
  function chart(selection) {
    selection.each(function(data) {
      ...
      **/
    	...
    	var svg= d3.select('#legend_div svg');

  	  svg.transition().duration(500)
  	      .attr('width', 500)
  	      .attr('height', 260)
      var wrap = svg.selectAll('g.legend').data([data]);
      ...
      series
          .attr('transform', function(d, i) {
             var length = d3.select(this).select('text').node().getComputedTextLength() + 90;
             xpos = newxpos;
      ...
      g.attr('transform', 'translate(' + /*(width - margin.right - maxwidth)*/5 + ',' + margin.top + ')');
