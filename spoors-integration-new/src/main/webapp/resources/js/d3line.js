...
function d3Line() {
  var margin = {top: 0, right: 0, bottom: 0, left: 0},
      ...
      x = d3.scale.linear(),
      y = d3.scale.linear(),
      dispatch = d3.dispatch("pointMouseover", "pointMouseout","pointMouseclick"),
      x0, y0;
  ...
  function chart(selection) {
    selection.each(function(data) {
    	...
    	var date=['','2013-12-15','2013-12-16','2013-12-17','2013-12-18','2013-12-19','2013-12-20','2013-12-21'];	
      var seriesData = data.map(function(d) { return d.data });
      var  specData= data.map(function(d) { return d.specId });
      x0 = x0 || x;
      y0 = y0 || y;
      ...
      var vertices = d3.merge(data.map(function(line, lineIndex) {
            return line.data.map(function(point, pointIndex) {
              var pointKey = line.specId + '-' + point[0];
              return [x(point[0]), y(point[1]), lineIndex, pointIndex,line.specId]; //adding series index to point because data is being flattened
            })
          })
      );
      ...
      var voronoi = d3.geom.voronoi(vertices).map(function(d,i) {
          return { 'data': d, 'series': vertices[i][2], 'point': vertices[i][3],'specId':vertices[i][4] }
      });
...
//              date:d.date,
      ...
      dispatch.on('pointMouseclick.point', function(d) { 
    	  
    	  var clicked=[];
    	  clicked.push(d.spec);
    	  clicked.push(d.pointIndex);
    	  callFormSearch(clicked);
//          wrap.select('.line-' + d.seriesIndex + ' .point-' + d.pointIndex)
//            .classed('hover', false);
        });
