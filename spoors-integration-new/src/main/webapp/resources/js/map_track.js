...
function setImagePaths(basePath){
	...
	iconCheckIn=iconBasePath + 'iconcheckin.png';
	iconCheckOut=iconBasePath + 'iconcheckout.png';
...
function initialize() {
	...
	for (var i = 0; i < latLongs.length; i++) {
		...
		if(latLongs[i][2] == 0){
		...
		else if(latLongs[i][2] == 11 || latLongs[i][2] == 13){
			var marker = null;
			if(latLongs[i][3] == 0){
		 ...
		 if(latLongs[i][2] == 14){
			 var marker = getMarker(latLongs[i][1], iconCheckIn, 'CustomerCheckIn');
				bindInfoWindow(marker, latLongs[i][0], new google.maps.InfoWindow);
				if(latLongs[i][3] == 1){
					var marker = null;
					marker = getMarker(latLongs[i][4], iconCheckOut, 'CustomerCheckOut');
					bindInfoWindow(marker, latLongs[i][5], new google.maps.InfoWindow);
				}	
			}
	}
	...
	for (var i = 0; i < latLongs.length; i++) {
		...
		var latLong=latLongs[i][1];
			if(latLong){
				flightPathPontsForWorkroute[j]=latLong;
			j++;
			}
	...
	flightPath3 = new google.maps.Polyline({
 	 	path: flightPathPontsForWorkroute,
 	 	strokeColor: '#B22222',
 	 	strokeOpacity: 1.0,
 	 	strokeWeight: 2,
 	 	icons : [arrow]
