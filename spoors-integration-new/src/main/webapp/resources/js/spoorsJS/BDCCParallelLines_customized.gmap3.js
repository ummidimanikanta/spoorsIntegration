...
function BDCCParallelLines(points, color, weight, opacity, gapPx, _type, _editable) {
  ...
  //this.zoomListener = google.maps.event.addListener(this.map, 'center_changed', zoomRecalc);
  this._type = _type;
...
BDCCParallelLines.prototype.onAdd = function() {
  this.setProjection();
  var foo = this;
  var zoomRecalc = function(event) {
    
   clearTimeout(refreshId);
	refreshId = setTimeout(function(e){
		//alert("zoom event triggered");
	     //var desiredRadiusInInMeters = ${gridFencingDistance};
		 //var desiredRadiusInInMeters = 50;
		  
		 var gridSearchId=$("#gridSearchId").val();
		 //alert("gridSearchId = "+gridSearchId);
		 if(isNotNull(gridSearchId)){
			  
			  foo.onRemove();
			  foo.setProjection();
			 //var desiredRadiusInInMeters = fencingWidthInMeters;
		     //var radiusInPixel = getRadiusInPixels(desiredRadiusInInMeters); 
		     
		     //alert("radiusInPixel = "+radiusInPixel);
		     
		     //geofence.setMap(null);
		     //onLoad(radiusInPixel);
			 removeGridFromMap(grids);
	         removePloyLine(polyline);
			 geofence.setMap(null);
			 var arr = getLatLongArrayByGridId(gridSearchId);
			 var radiusInPixel = getRadiusInPixels(fencingWidthInMeters);
	         drawGrid(arr,radiusInPixel);
		 }
	}, 500);
  };
  ...
  //this.zoomListener = google.maps.event.addListener(this.map, 'center_changed', zoomRecalc);
   //this.zoomListener = google.maps.event.addListener(this.map, 'bounds_changed', zoomRecalc);
   this.zoomListener = google.maps.event.addListener(this.map, 'idle', zoomRecalc);
